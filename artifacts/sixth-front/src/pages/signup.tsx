      ...
      <section className="bg-white rounded-2xl shadow-sm border border-line p-6 md:p-10">
        {step === 1 && <form onSubmit={(event) => { event.preventDefault(); continueBasics(event.currentTarget); }} className="space-y-4"><h1 className="text-2xl font-display font-bold text-red">Business Basics</h1><p className="text-muted text-base">Let's start with the essential details about your eatery.</p>
          ...
          <div className="space-y-3 pt-2">
            <p className="text-[13px] leading-relaxed text-muted" data-testid="notice-intake-non-binding">Submitting this form requests information or a preliminary preview only. It does not create a services agreement, require either party to proceed, reserve pricing, register or transfer any domain, or authorize launch of services. Services begin only after the applicable agreement, Order Form, payment authorization, and onboarding requirements are accepted.</p>
            <p className="text-[13px] leading-relaxed text-muted" data-testid="notice-intake-privacy">By submitting this form, you authorize Sixth Front to use the information provided to evaluate your request, contact you, prepare a preliminary preview, and provide related onboarding information. See our <Link href="/privacy" className="underline underline-offset-2 hover:text-ink">Privacy Policy</Link> for details.</p>
          </div>
        ...
        {step === 5 && <div><h1 className="text-2xl font-display font-bold text-red mb-2">Merchant Services Agreement</h1>{agreement.isLoading ? <p className="text-lg">Loading agreement…</p> : agreement.isError || !agreement.data ? <p className="text-red text-lg">Unable to load agreement. Please try again later.</p> : <><p className="text-base text-muted mb-3">Please read the Master Merchant Services Agreement and Pricing Schedule below before accepting. Version <span data-testid="text-agreement-version" className="font-bold text-ink">{agreement.data.version}</span>.</p><div className="my-5 p-4 bg-paper border border-line rounded whitespace-pre-wrap text-base" data-testid="agreement-body"><b>{agreement.data.merchantServicesTitle}</b><p className="mt-2">{agreement.data.merchantServicesContent}</p><b className="block mt-4">{agreement.data.pricingScheduleTitle}</b><p className="mt-2">{agreement.data.pricingScheduleContent}</p></div><label className="flex gap-3 items-start text-base"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} data-testid="checkbox-agreement" className="mt-1 shrink-0" /><span>I have read and agree to the Master Merchant Services Agreement and Pricing Schedule shown above (Version {agreement.data.version}), and to the <Link href="/terms" className="underline underline-offset-2 hover:text-ink">Terms of Use</Link>, <Link href="/privacy" className="underline underline-offset-2 hover:text-ink">Privacy Policy</Link>, <Link href="/cookies" className="underline underline-offset-2 hover:text-ink">Cookie Notice</Link>, and <Link href="/sms-terms" className="underline underline-offset-2 hover:text-ink">SMS &amp; Email Terms</Link>. I understand I am entering into a binding agreement and consent to receive these records electronically.</span></label></>}{message && <p className="mt-4 text-red text-lg" data-testid="status-standard-signup-error">{message}</p>}<div className="flex justify-between mt-8"><button type="button" onClick={() => setStep(4)} data-testid="button-back-5">Back</button><button type="button" onClick={submitStandard} disabled={!accepted || standardSignup.isPending || !agreement.data} className={buttonClass} data-testid="button-create-storefront">{standardSignup.isPending ? 'Submitting...' : 'Create My Storefront'}</button></div></div>}
      </section>
      <div className="mt-10 pt-6 border-t border-line"><LegalFooter variant="muted" /></div>
    </main>
  </div>;
}